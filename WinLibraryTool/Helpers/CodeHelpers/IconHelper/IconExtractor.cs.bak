...
using WinLibraryTool.Helpers.CodeHelpers;
...
namespace WinLibraryTool.Helpers.CodeHelpers.IconHelper
{
	...
	public class IconExtractor : IDisposable
	{
		...
		public Icon GetIconAt(int index)
		{
			if (index < 0)
			{
				// index is a resource ID, convert it back to an index.
				index = IconIndexFromResourceId(index);
				if (index < 0)
				{
					throw new ArgumentOutOfRangeException("index", index, "Resource id not found.");
				}
			}

			if (index >= this.IconCount)
			{
				if (this.IconCount > 0)
		...
		/// <summary>
		/// Converts an icon resource ID into an index.
		/// </summary>
		/// <param name="resourceId">Icon resource ID.</param>
		/// <returns>The index, if it exists, otherwise, -1.</returns>
		private int IconIndexFromResourceId(int resourceId)
		{
			int positiveResId = resourceId * -1;

			for (int i = 0; i < IconNamesList.Count; i++)
			{
				if (IconNamesList[i].Id == positiveResId)
				{
					return i;
				}
			}
		...
		private Icon GetIconFromLib(int index)
		{
			...
			using (MemoryStream destStream = new MemoryStream())
			{
				...
				GroupIconDir grpDir = StreamHelper.ReadStructure<GroupIconDir>(inputStream);
				...
				StreamHelper.WriteStructure<IconDir>(destStream, grpDir.ToIconDir());
				for (int i = 0; i < numEntries; i++)
				{
					//Read the GroupIconDirEntry.
					GroupIconDirEntry grpEntry =  StreamHelper.ReadStructure<GroupIconDirEntry>(inputStream);

					//Write the IconDirEntry.
					destStream.Seek(IconInfo.SizeOfIconDir + i * IconInfo.SizeOfIconDirEntry, SeekOrigin.Begin);
					StreamHelper.WriteStructure<IconDirEntry>(destStream, grpEntry.ToIconDirEntry(iconImageOffset));
